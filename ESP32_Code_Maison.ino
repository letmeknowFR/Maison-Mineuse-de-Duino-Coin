//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//Fonctionne avec core ESP32 a partir de 2.0
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

// Change this to your WiFi name
const char *wifi_ssid = "ARTICHAUT 3";
// Change this to your WiFi wifi_password
const char *wifi_password = "artichaut16";
// Change this to your Duino-Coin username
const char *username = "Fablmk";
// Change this if you want a custom rig identifier
const char *rig_identifier = "House-Artichaut";
...
#include <OneWire.h> 
//ATTENTION Modifier le fichier OneWire OneWire_direct_gpio.h
// remplacer tous les rtc_gpio_desc en rtc_io_desc
...
byte task1_state = 0;
byte task2_state = 0;
byte task1_last_state = 10;
byte task2_last_state = 10;
#define SEUIL_TEMP 40                           // Temperature de seuil
#define HYSTERESIS 1                            // Gain de dynamisme sur la mesure
#define DEEP_SLEEP_TIME 30                      // Temps de sleep
String temp_txt = "";
String temp_txt_interne = "";
...
void loop() {
  ...
  if (tempC != DEVICE_DISCONNECTED_C) {
    ...
    temp_txt = String(tempC, 2) + "°C / " + SEUIL_TEMP + "°C";
  } else {
    Serial.println("Error: Could not read temperature data");
  }
  if (task1_state != task1_last_state || task2_state != task2_last_state || tempC != DEVICE_DISCONNECTED_C) {
    initDisplay();
    display.clear();
    switch (task1_state) {
      case 0:
        printBuffer("C1:Connecting", 0, 0, 12);
        break;
      case 1:
        printBuffer("C1:Connected", 0, 0, 12);
        break;
      case 2:
        printBuffer("C1:Asking", 0, 0, 12);
        break;
      case 3:
        printBuffer("C1:Job received", 0, 0, 12);
        break;
      case 4:
        printBuffer("C1:Job done", 0, 0, 12);
        break;
      case 5:
        printBuffer("C1:Connexion lost", 0, 0, 12);
        break;
      case 6:
        printBuffer("C1:Job Fail", 0, 0, 12);
        break;
      case 7:
        printBuffer("RESTART", 0, 0, 12);
        break;
      case 10:
        printBuffer("C1: OFF", 0, 0, 12);
        break;
    }
    switch (task2_state) {
      case 0:
        printBuffer("C2:Connecting", 0, 15, 12);
        break;
      case 1:
        printBuffer("C2:Connected", 0, 15, 12);
        break;
      case 2:
        printBuffer("C2:Asking", 0, 15, 12);
        break;
      case 3:
        printBuffer("C2:Job received", 0, 15, 12);
        break;
      case 4:
        printBuffer("C2:Job done", 0, 15, 12);
        break;
      case 5:
        printBuffer("C2:Connexion lost", 0, 15, 12);
        break;
      case 6:
        printBuffer("C2:Job Fail", 0, 15, 12);
        break;
      case 7:
        printBuffer("RESTART", 0, 15, 12);
        break;
      case 10:
        printBuffer("C2: OFF", 0, 15, 12);
        break;
    }
    printBuffer(temp_txt, 0, 30, 12);
    temp_txt_interne = (temprature_sens_read() - 32) / 1.8;
    temp_txt_interne = temp_txt_interne + "°C";
    printBuffer(temp_txt_interne, 0, 45, 12);
    display.display();
    task1_last_state = task1_state;
    task2_last_state = task2_state;
  }
  if (tempC >= SEUIL_TEMP + HYSTERESIS) { // S'il fait trop chaud
    if (task1_state != 10) { // Verifier que la tâche est active
      vTaskDelete(Task1);
      task1_state = 10;
      vTaskDelete(Task2);
      task2_state = 10;
    ...
    if (task1_state == 10 && tempC <= SEUIL_TEMP - HYSTERESIS) { // Verifier que la tâche est inactive
      WiFi.mode(WIFI_STA); // Setup ESP in client mode
      WiFi.begin(wifi_ssid, wifi_password); // Connect to wifi
      esp_task_wdt_init(WDT_TIMEOUT, true); // Init Watchdog timer
